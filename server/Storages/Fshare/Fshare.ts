...
const debug = require("debug")("eMCloud::Fshare");

import { EventEmitter } from 'events';

export class Fshare extends EventEmitter {
    creds;
    ...
    static callbackHandler(query, callback) {
        ...
        var storage = fshare({ email: query.username, password: query.password, keepalive: false }, (err) => {
            if (err) {
                debug("Error: " + err);
                callback(0);
            }
    ...
    public uploadFile(readStream, totalSize, mime, filename, parent?, callback?) {
        ...
        var self = this;
        var storage = fshare({ email: this.creds.email, password: this.creds.password, keepalive: false }, (err) => {
            if (err) {
